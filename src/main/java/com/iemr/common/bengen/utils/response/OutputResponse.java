    ...
    public void setError(Throwable thrown) {
        ...
        switch (thrown.getClass().getSimpleName()) {
            ...
            case "ConstraintViolationException":
                ...
                status = "Failed with critical errors at " + currDate + ".Please try after some time. "
            ...
            case "ConnectIOException":
                ...
                status = "Failed with connection issues at " + currDate + "Please try after some time. "
                    + "If error is still seen,  contact your administrator.";
                errorMessage = thrown.getMessage();
                break;
            case "JDBCException":
                this.statusCode = ENVIRONMENT_EXCEPTION;
                status = "Failed with DB connection issues at " + currDate + ". Please try after some time. "
                    + "If error is still seen,  contact your administrator.";
                errorMessage = thrown.getMessage();
                break;
            default:
                this.statusCode = GENERIC_FAILURE;
                status = "Failed with " + thrown.getMessage() + " at " + currDate
                    + ".Please try after some time. If error is still seen, contact your administrator.";
                errorMessage = thrown.getMessage();
                break;
        }
        logger.error("Failure happend with " + thrown.getMessage() + "at " + currDate, thrown);
    }
